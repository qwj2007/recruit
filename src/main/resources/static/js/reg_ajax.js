...
function reg_checkAjax(id) {
    ...
    if (id == "username1") {
        if (obj == "") {
            ...
            return true;
            //$.post("Ajax/ValidateAjax.ashx=checkname", { username: obj }, function (data) {
            //    if (data == 0) {
            //        msg = '填写正确！';
            //        update_html(id, "1", msg);
            //        return true;
    ...
    if (id == "email1" || id == "email3") {
        ...
        if (obj == "") {
            ...
            update_html(id, "1", msg);
            return true;
            //$.post("Ajax/checkAjax.ashx?op=email", { email: obj }, function (data) {
            //    if (data == 0) {
            //        $("#def" + id).hide();
            //        msg = "输入成功！";
            //        update_html(id, "1", msg);
...
function sendmsg() {
    ...
    if (date == 1 && send == 0) {
        $.post(weburl + "/index.php?m=ajax&c=regcode", { moblie: moblie }, function (data) {
            if (data == 0) {
                layer.msg('手机不能为空！', 2, 8); return false;
            } else if (data == 1) {
...
function check_user(id) {
    ...
    $.ajax({
        url: "Ajax/LoginAjax.ashx?op=reg&yhm=" + username + "&mm=" + password + "&email=" + email + "",
        type: "post",
        async: false,
        dataType: "text",
        success: function (msg) {
            if (msg == "ok") {
                //alert("注册成功！");
                window.location.href = "/Center/BasicEdit.aspx";
            }
            else {
                //alert("注册失败！");
                if (msg == "1") {
...
//    } else {
//        $.post(weburl + "/index.php?m=register&c=regsave", { username: username, password: password, email: email, moblie: moblie, moblie_code: moblie_code, unit_name: unit_name, address: address, authcode: authcode, usertype: usertype, name: name, linkman: linkman, codeid: id }, function (data) {
//            layer.close(loadi);
//            var data = eval('(' + data + ')');
...
//            } else {
//                if (status == 1) {
//                    //注册成功,后台校区设置开启不审核直接跳转到会员中心
//                    layer.msg(msg, 2, 9, function () { window.location.href = weburl + "/member/index.php"; });
//                } else if (status == 8) {
//                    layer.msg(msg, 2, 8, function () { check_codes(); });
//                } else if (status == 7) {
//                    //注册成功,后台校区设置开启审核跳转到待审核页面
...
function check_login(url) {
    ...
    $.post(url, { username: username, password: password, path: path, loginname: loginname, authcode: authcode }, function (data) {
        ...
        if (jsonObject.error == '3') {//UC登录激活 
            ...
            window.location.href = jsonObject.url; return false;
